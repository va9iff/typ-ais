...
export class  VSorgu extends VLitElement {
	...
	render(){
		...
		ru<input type="checkbox" name="" @change=${e=>{
			// e.target.checked = !e.target.checked
		...
		<br>
		<br>
		Cinsiyyət <select name="" id="" @change=${
		...
		<br>
		<br>
		Minimum İş Təcrübəsi
		<input type="number" @input=${e=>{
		...
		<table class="table">
			...
			<tbody>
					...
					.filter(t=>this.exp>0?t.exp>this.exp:true)
					.map((teacher)=>
				html`
				<tr>
